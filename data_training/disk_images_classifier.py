...
class DiskImagesClassifier:
    ...
    def create_model(self, train_generator, val_generator, test_generator):
    ...
    def use_model(self, generator):
        model = self.__load_model()
        self.__use_classification_model(model, generator)
        pass
    ...
    def __create_classification_model(self):
        ...
        model = self.__train_model(model, self.train_generator, self.val_generator, self.test_generator)

        print(f'{self.log_name} Finished image classification')
        return model

    def __use_classification_model(self, model: Model, generator: DirectoryIterator):
    ...
    def __train_model(self, model: Model, train_generator: DirectoryIterator, val_generator: DirectoryIterator,
                      test_generator: DirectoryIterator):
        print(f'{self.log_name} Started training on training set')
        start_time = timer()

        model, history = self.__fit_model_with_generator(model, train_generator, val_generator, epochs=self.epochs)
    ...
    @staticmethod
    def __fit_model_with_generator(model: Model, train_generator, val_generator,
                                   epochs=20):
        history = model.fit(train_generator, epochs=epochs, validation_data=val_generator, shuffle=True)
        return model, history

    @staticmethod
    def __evaluate_model_with_generator(model: Model, test_generator: DirectoryIterator):
        model.evaluate(test_generator)

    def __predict_model_with_generator(self, model: Model, test_generator: DirectoryIterator):
        ...
        y_dataset = test_generator.labels
